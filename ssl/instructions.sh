...
openssl req -passin pass:1111 -new -key ssl/server.key -out ssl/server.csr -subj "/CN=${SERVER_CN}" -config ssl/ssl.cnf

# Step 4: Sign the certificate with the CA we created (it's called self signing) - server.crt
openssl x509 -req -passin pass:1111 -days 3650 -in ssl/server.csr -CA ssl/ca.crt -CAkey ssl/ca.key -set_serial 01 -out ssl/server.crt -extensions req_ext -extfile ssl/ssl.cnf
